...
module.exports = async (client, player, track, payload) => {
  ...
  const thing = new MessageEmbed()
    .setDescription(`${i18n.__("cmd.play.embed")}\n [${track.title}](https://discord.gg/FbCsKCjUWR)  \n\n\`🕧 ${convertTime(track.duration)}\n\`
<:req:1199978800923037757> **Requester**\n${track.requester.tag}`)
    
    .setThumbnail(
      track?.thumbnail ??
        (await client.manager.getMetaThumbnail(track?.uri ?? queue.uri)))
    .setFooter("THE EXTREMEZ CODER <3")
       .setColor(client.embedColor)
    .setTimestamp()
  const But1 = new MessageButton().setEmoji("<:Volume_less:1199969731235549224>").setCustomId("vdown").setStyle("SECONDARY");
    
   const But2 = new MessageButton().setEmoji("<:icons_stop_button:1199968797369565275>").setCustomId("stop").setStyle("SECONDARY");

   const But3 = new MessageButton().setCustomId("pause").setEmoji("<:icon_pause:1199976570241167470>").setStyle("SECONDARY");

   const But4 = new MessageButton().setCustomId("skip").setEmoji("<:icon_skip:1199969957472124958>").setStyle("SECONDARY");
    
   const But5 = new MessageButton().setCustomId("vup").setEmoji("<:Volume_up:1199969812999311401>").setStyle("SECONDARY");
