     ...
     let regex = new RegExp(`^<@!?${client.user.id}>`);
      ...
      if(command.voteonly){
        ...
        if(!vote){
          ...
          .setDescription(`<:tec:1199357089395527723> | **This Is A Vote Required Command So Vote Us Now By [Clicking Here](https://top.gg/bot/${client.user.id}/vote)**`)
          .setColor(client.embedColor)
   ...
   if (!channel.permissionsFor(message.guild.me)?.has(Permissions.FLAGS.EMBED_LINKS) && client.user.id !== userId) {
    ...
    if (command.owner) {
     if (!owner.includes(message.author.id)) {
        ...
        embed.setDescription("<:cross:1200733725344944178> | Only My Developers Can Use This Command");
        return message.channel.send({embeds: [embed]});
    }
    }
